...
/* =========================
تشخیص اینکه داخل اپ نصب‌شده (Capacitor) هستیم یا داخل مرورگر معمولی
========================= */
...
/* شناسه‌ی عددی یکتا برای هر آلارم (Local Notifications به عدد صحیح نیاز دارد) */
function nextNotifId() {
    let seq = Number(localStorage.getItem("mk_notif_seq") || "0");
    seq = (seq + 1) % 2000000000;
    localStorage.setItem("mk_notif_seq", String(seq));
    return seq + 1000;
}

/* مهاجرت کارهای قدیمی که notifId ندارند */
(function migrateNotifIds() {
    let changed = false;
    tasks.forEach(task => {
        if (!task.notifId) {
            task.notifId = nextNotifId();
            changed = true;
        }
    });
    if (changed) save();
...
function toggleTask(index) {
    ...
    task.done = !task.done;
    task.doneAt = task.done ? localDate() : null;
...
async function requestNotifications() {
    ...
    /* حالت اپ واقعی (اندروید/iOS) — از پلاگین Capacitor استفاده می‌کنیم */
    ...
    /* حالت مرورگر معمولی — فقط برای تست روی کامپیوتر */
    ...
    try {
        ...
        if (permission === "granted") {
            alert("اعلان‌ها فعال شدند 🔔 (این حالت فقط برای تستِ داخل مرورگر است — روی گوشی از حالت اپ استفاده می‌شود)");
            restoreAlarms();
        } else {
            alert("اجازه اعلان داده نشد.");
        }
...
/* =========================
جلوگیری از اعلان تکراری (فقط حالت مرورگر)
========================= */

function alarmKey(notifId, time) {
    return "mk_alarm_" + notifId + "_" + time.getTime();
}

function alarmAlreadyFired(notifId, time) {
    return localStorage.getItem(alarmKey(notifId, time)) === "1";
}

function markAlarmFired(notifId, time) {
    localStorage.setItem(alarmKey(notifId, time), "1");
}

function showBrowserNotification(task, time) {
    if (!("Notification" in window)) return;
    if (Notification.permission !== "granted") return;
    if (alarmAlreadyFired(task.notifId, time)) return;

    try {
        new Notification("مدیریت کار 🔔", {
            body: task.title + "\nالان زمان انجام این کاره!",
            tag: "task-alarm-" + task.notifId,
            requireInteraction: true
        });

        markAlarmFired(task.notifId, time);
    } catch (error) {
...
/* =========================
زمان‌بندی آلارم
- داخل اپ (Capacitor): با Local Notifications، سیستم‌عامل خودش نگه می‌دارد
  و حتی وقتی اپ کاملاً بسته باشد هم فایر می‌شود.
- داخل مرورگر معمولی: فقط برای تست، و تا وقتی تب باز است کار می‌کند.
========================= */

async function scheduleAlarm(task) {
    if (!task || !task.date || !task.time || task.done) return;
    ...
    if (isNative() && LocalNotifications) {
        try {
            ...
            console.error("خطا در زمان‌بندی آلارم:", error);
        }
        return;
    }
    ...
    setTimeout(() => {
        showBrowserNotification(task, alarmTime);
    }, delay);
...
/* =========================
لغو آلارم (وقتی کار حذف یا انجام‌شده می‌شود)
========================= */

async function cancelAlarm(task) {
    if (!task || !task.notifId) return;
    ...
    if (isNative() && LocalNotifications) {
        try {
            await LocalNotifications.cancel({
                notifications: [{ id: task.notifId }]
            });
        } catch (error) {
            console.error("خطا در لغو آلارم:", error);
        }
    }
}

/* =========================
برگرداندن آلارم‌ها (هنگام باز شدن دوباره‌ی اپ)
========================= */
...
function clearTasks() {
    ...
    tasks.forEach(task => cancelAlarm(task));
